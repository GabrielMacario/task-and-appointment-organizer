...
// // Configurando o intervalo para recarregar a página a cada minuto
...
const atualizarListaDeAtividades = () => {
  ...
  if (atividades.length == 0) {
    section
    .innerHTML = `<p>Nenhuma atividade cadastrada</p>`
    return
  }
  ...
  for (let atividade of atividades) {
    section
    .innerHTML += criarItemDeAtividade(atividade);
  }
};
...
const criarDiasSelecao = () => {
    ...
    for (let i = 0; i < 31; i++) {
        ...
        const diaTexto = dia.format("DD / MMMM");
        diasSelecao.push(`<option value="${diaFormatado}">${diaTexto}</option>`);
    }
    document.querySelector('select[name="dia"]').innerHTML = diasSelecao.join('');
}
